...
BEAVER = 'beaver-test'
HUMAN_DIR = '/path/to/work/dir/results/HEK293T_simulation/results/'
HUMAN_BEAVER_DIR = HUMAN_DIR + BEAVER
HUMAN_SAMPLE_SIZE_LIST = [2, 5, 10, 20, 30, 50, 80, 100, 150, 192]
#HUMAN_SAMPLE_SIZE_LIST = [2]
MOUSE_DIR = '/path/to/work/dir/results/Mouse-Fibroblast_simulation/results/'
...
TRAIN_DIR = f'{TRAIN_BASE_DIR}/simDataset-{BEAVER}-meta+specific-final'
TRAIN_CHRM = ['1','2','3','4','5','6','7','8','9']
os.makedirs(TRAIN_DIR, exist_ok=True)

HUMAN_SAMPLE_SIZE_PLOT_LIST = [5, 10, 30, 50, 100, 192]
MOUSE_SAMPLE_SIZE_PLOT_LIST = [5, 30, 50, 100, 200, 369]

# %%
def process_tmap_files_other_chrs(tool, sample_size, base_dir, species):
    """
    ...
    for cell_id in range(start_id, end_id + 1):
        ...
        gt_file = f"/path/to/work/dir/results/{species}_simulation/ground_truth/{gt_file_id}.list"
        ...
        with open(tmap_file, 'r') as tmap:
            ...
            for line in tmap:
                ...
                specific_gt = ref_id in cell_specific_gt
                label_general = class_code == '='
                label_specific = label_general and specific_gt
        ...
        output_dir = f"{TRAIN_BASE_DIR}/simDataset-tools-otherchrs/{tool}/{species}"
...
def process_all_tools_other_chrs(species):
    ...
    for tool in tools:
        ...
        for sample_size in sample_sizes:
            _, cell_stats = process_tmap_files_other_chrs(tool, sample_size, base_dir, species)
            all_stats.extend(cell_stats)
        
        # Save statistics to CSV
        output_dir = f"{TRAIN_BASE_DIR}/simDataset-tools-otherchrs/{tool}"
        os.makedirs(output_dir, exist_ok=True)
...
def get_meta_feature_vector(v, sample_size):
    ...
    #features.extend(calc_stats([float(count) / num_junctions for count in cell_junction_counts]))
    
    #from collections import Counter
    #count_frequency = Counter(cell_junction_counts)
    #most_common_junc_count, most_common_junc_frequency = count_frequency.most_common(1)[0]
    #most_common_junc_count = round(float(most_common_junc_count) / num_junctions, 3)
    #most_common_frequency_ratio = round(float(most_common_junc_frequency) / cells_support, 3)
    
    #features.append(most_common_junc_count)
    #features.append(most_common_frequency_ratio)
...
# %%
def get_specific_labels_from_tmap(tmap_file, cell_specific_gt):
    """Extract labels from tmap file."""
    labels = {}
    with open(tmap_file, 'r') as tmap:
        next(tmap)  # skip the header
...
def combine_general_to_specific(data_dir, data_source, sample_size, train_chrm, meta_prediction):
    """Process data for a specific sample size."""
    all_data = []
    ...
    for cell_id in range(1, sample_size + 1):
        ...
        gt_file = f"/path/to/work/dir/results/{data_source}_simulation/ground_truth/{cell_id}.list"
        ...
        labels = get_specific_labels_from_tmap(tmap_file, cell_specific_gt)
    ...
    final_columns = [
        'data_source', 'sample_size', 'cell_id', 'transcript_id', 'chr',
        'input_genes', 'input_transcripts', 'output_transcripts',
        'supporting_junctions', 'ratio_supporting_junctions',
        'bottleneck_coverage', 'highest_coverage', 
        'extendable_score', 'num_junctions',
        'min_comp_cov', 'max_comp_cov', 'median_comp_cov', 'mean_comp_cov', 'std_comp_cov',
        'min_junc_cov', 'max_junc_cov', 'median_junc_cov', 'mean_junc_cov', 'std_junc_cov',
    ...
    # Sort by y_prob_general in decreasing order
    #df = df.sort_values('y_prob_general', ascending=False)
...
def process_and_save_specific_data(data_dir, data_source, sample_size_list, train_chrm, meta_prediction):
    """Process and save data for all sample sizes."""
    for sample_size in sample_size_list:
...
def compare_with_other_tools(data_source, sample_size_list, tools, predictions_dir, plot_data_dir):
    ...
    for sample_size in sample_size_list:
        ...
        for tool in tools:
            ...
            tool_csv_path = os.path.join(TRAIN_BASE_DIR, "simDataset-tools-otherchrs", tool, f"{data_source}_statistics.csv")
...
def process_meta_models(data_source, load_size_list, sample_size_list):
    ...
    meta_model, meta_model_filename, meta_yprob, meta_train_yprob = train_and_evaluate_meta_model(
        ...
        f'model_sim{data_source}_{BEAVER}_meta'
    )
...
def process_cell_specific_models(data_source, load_size_list, sample_size_list, meta_train, meta_test):
    ...
    process_and_save_specific_data(beaver_dir, data_source, load_size_list, TRAIN_CHRM, combined_meta)
    ...
    model, model_filename, yprob = train_and_evaluate_specific_model(
        ...
        f'model_sim{data_source}_{BEAVER}_specific'
    )
...
evaluate_and_save_comparison('HEK293T', HUMAN_SAMPLE_SIZE_LIST, HUMAN_SAMPLE_SIZE_PLOT_LIST)
evaluate_and_save_comparison('Mouse-Fibroblast', MOUSE_SAMPLE_SIZE_LIST, MOUSE_SAMPLE_SIZE_PLOT_LIST)
